...
export interface OrderInTransitProps {
  customerName: string;
  orderNumber: string;
  orderDate: Date;
  productName: string;
...
export default function OrderInTransitEmail(props: OrderInTransitProps) {
  const {
    customerName,
    orderNumber,
    orderDate,
  ...
  const previewText = `Your order #${orderNumber} is in transit!`;
    ...
    <Html>
      ...
      <Body style={styles.body}>
        <Container style={styles.container}>
          ...
          <Text style={styles.heading}>🚚 Your order is on the way, {firstName}!</Text>
          <Text style={styles.muted}>
            Great news! Your package is now moving through the shipping network and is on its way to
            you. You can track its progress in real-time.
          </Text>
          ...
          <Section style={{ ...styles.card, marginTop: '14px' }}>
            <Text style={{ ...styles.heading, margin: '0 0 12px' }}>Item in transit</Text>
            {productImage && (
              <Img
                src={productImage}
                alt={productName}
